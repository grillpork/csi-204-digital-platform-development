...
function r2Config() {
  ...
  return R2_ACCOUNT_ID && R2_ACCESS_KEY_ID && R2_SECRET_ACCESS_KEY && R2_BUCKET_NAME && R2_PUBLIC_URL
    ? { R2_ACCOUNT_ID, R2_ACCESS_KEY_ID, R2_SECRET_ACCESS_KEY, R2_BUCKET_NAME, R2_PUBLIC_URL } : null;
}
...
export async function uploadImage(file, folder = "designs") {
  ...
  if (config) {
    ...
    await client.send(new PutObjectCommand({ Bucket:config.R2_BUCKET_NAME, Key:key, Body:body, ContentType:file.type, CacheControl:"public, max-age=31536000, immutable" }));
...
export async function deleteImage(key) {
  ...
  if (config) {
    const client = new S3Client({ region:"auto", endpoint:`https://${config.R2_ACCOUNT_ID}.r2.cloudflarestorage.com`, credentials:{ accessKeyId:config.R2_ACCESS_KEY_ID, secretAccessKey:config.R2_SECRET_ACCESS_KEY } });
    await client.send(new DeleteObjectCommand({ Bucket:config.R2_BUCKET_NAME, Key:key }));
  } else {
